...
async def catg_list():
    ...
    select = f"company_name,address,contact_person,phone_no,email_id,max_user,max_outlet,DATE_FORMAT(last_billing, '%d-%m-%Y') AS last_billing,DATE_FORMAT(DATE_ADD(last_billing, INTERVAL 1 YEAR),'%d-%m-%Y') AS next_bill_date,sales_person,active_flag"
    table_name = "md_company"
