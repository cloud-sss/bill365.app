...
function ManageLocations() {
  ...
  useEffect(() => {
    setDataSet(
      response?.data?.msg?.filter((e) =>
        e.location_name.toLowerCase().includes(search?.toString().toLowerCase())
      )
    );
  }, [search]);
  ...
  const handleAdd = (data) => {
    ...
    setCalled(true);
    if (data)
      callApi("/admin/S_Admin/add_edit_location", 1, {
        sl_no: 0,
        location_name: data,
        user_id: userId,
      });
  ...
  return (
    <div className="py-1 w-full">
      ...
      <Dialog
        ...
        <DialogContent>
          <DialogContentText>
            By clicking on the add button, you will be able to add a location in
            the table below.
          </DialogContentText>
          <TextField
            autoFocus
            ...
            variant="standard"
            value={unit}
            onChange={(loc) => setUnit(loc.target.value)}
          />
        </DialogContent>
        <DialogActions>
          <Button
